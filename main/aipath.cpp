...
int create_path_points(object *objp, int start_seg, int end_seg, point_seg *psegs, short *num_points, int max_depth, int random_flag, int safety_flag, int avoid_seg)
{
	...
	int		l_num_points;
	bool	end_seg_is_qtail_end = false;

// -- mprintf((0, "cpp: frame = %4i obj %3i, psegs = %5i\n", FrameCount, objp-Objects.data(), psegs-Point_segs));
#if PATH_VALIDATION
	validate_all_paths();
#endif
...
//	}
	...
	while (cur_seg != end_seg) 
	{
		...
		if (random_flag && currentGame != G_DESCENT_1)
			if (P_Rand() < 8192)
				create_random_xlate(random_xlate);
...
cpp_done1: ;
	...
	// -- N_selected_segs = 0;
	#endif
//printf("Object #%3i, start: %3i ", objp-Objects.data(), psegs-Point_segs);
	while (qtail >= 0) {
...
int polish_path(object *objp, point_seg *psegs, int num_points)
{
	...
	if (num_points <= 4 || currentGame == G_DESCENT_1)
		return num_points;
...
void create_d1_path(object* objp)
{
	ai_static* aip = &objp->ctype.ai_info;
...
//mprintf((0, "Creating path to player.\n"));
	...
	ailp->goal_segment = (currentGame == G_DESCENT_1 ? ConsoleObject->segnum : Believed_player_seg);

	start_seg = objp->segnum;
	end_seg = ailp->goal_segment;

	// mprintf((0, "Creating path for object #%i, from segment #%i to #%i\n", objp-Objects.data(), start_seg, end_seg));
...
//mprintf((0, "Creating %i segment path.\n", path_length));
	...
	if (Ai_local_info[objp-Objects.data()].previous_visibility && currentGame == G_DESCENT_2) {
		if (aip->path_length) {
...
void move_object_to_goal(object *objp, vms_vector goal_point, int goal_seg)
{
	...
	if (aip->cur_path_index <= 0) {
		...
		aip->cur_path_index = (currentGame == G_DESCENT_1 ? 0 : 1);
		aip->PATH_DIR = 1;
	} else if (aip->cur_path_index >= aip->path_length - (currentGame == G_DESCENT_1 ? 0 : 1)) {
		if (aip->behavior == AIB_STATION) {
			// mprintf((0, "Object #%i, creating path back to station.\n", objp-Objects.data()));
			create_path_to_station(objp, 15);
			if (aip->path_length == 0 && currentGame != G_DESCENT_1) {
				ai_local		*ailp = &Ai_local_info[objp-Objects.data()];
				ailp->mode = AIM_STILL;
			}
			return;
		...
		aip->cur_path_index = aip->path_length - (currentGame == G_DESCENT_1 ? 1 : 2);
		aip->PATH_DIR = -1;
...
// -- too much work -- 	fq.thisobjnum			= objp-Objects.data();
...
// mprintf((0, "Obj %i, dist=%6.1f index=%i len=%i seg=%i pos = %6.1f %6.1f %6.1f.\n", objp-Objects.data(), f2fl(vm_vec_dist_quick(&objp->pos, &ConsoleObject->pos)), aip->cur_path_index, aip->path_length, objp->segnum, f2fl(objp->pos.x), f2fl(objp->pos.y), f2
	...
	if (((size_t)aip->hide_index + aip->path_length > Point_segs_free_ptr - Point_segs) && (aip->path_length>0)) {
		if (currentGame == G_DESCENT_2)
			Int3();	// [DW] In D1, this never tripped.
	...
	if (ailp->mode == AIM_RUN_FROM_OBJECT) {
		if ((player_visibility == 0) && (ailp->player_awareness_type == 0)) {
			...
			return;
		} else if (currentGame == G_DESCENT_1 || !(FrameCount ^ ((objp-Objects.data()) & 0x07))) {		//	Done 1/8 frames.
	...
	while ((dist_to_goal < threshold_distance) && !forced_break) {
		...
		if ((aip->cur_path_index >= aip->path_length) || (aip->cur_path_index < 0)) {
			...
			if (aip->behavior == AIB_FOLLOW) {
				...
				create_path_to_station(objp, 15);
				if ((aip->hide_segment != Point_segs[aip->hide_index+aip->path_length-1].segnum) || (aip->path_length == 0 && currentGame == G_DESCENT_2)) {
					ailp->mode = AIM_STILL;
				} else {
					//--Int3_if(((aip->cur_path_index >= 0) && (aip->cur_path_index < aip->path_length)));
				}
				...
				create_n_segment_path(objp, AVOID_SEG_LENGTH, ConsoleObject->segnum);
				ailp->mode = AIM_RUN_FROM_OBJECT;	//	It gets bashed in create_n_segment_path
				if (aip->path_length < 1 && currentGame == G_DESCENT_2) {
					create_n_segment_path(objp, AVOID_SEG_LENGTH, ConsoleObject->segnum);
					ailp->mode = AIM_RUN_FROM_OBJECT;	//	It gets bashed in create_n_segment_path
...
void ai_path_set_orient_and_vel(object *objp, vms_vector goal_point, int player_visibility, vms_vector* vec_to_player)
{
	...
	if ((Ai_local_info[objp-Objects.data()].mode == AIM_RUN_FROM_OBJECT) || (robptr->companion == 1) || (objp->ctype.ai_info.behavior == AIB_SNIPE)) {
		if (Ai_local_info[objp-Objects.data()].mode == AIM_SNIPE_RETREAT_BACKWARDS) {
...
void ai_path_garbage_collect(void)
{
	...
	obj_path* object_list = new obj_path[Objects.size()];
	...
	for (objnum=0; objnum <= Highest_object_index; objnum++) {
		...
		if ((objp->type == OBJ_ROBOT) && ((objp->control_type == CT_AI) || (objp->control_type == CT_MORPH && currentGame == G_DESCENT_2))) {
			ai_static	*aip = &objp->ctype.ai_info;
...
void player_path_set_orient_and_vel(object *objp, vms_vector *goal_point)
{
	...
	max_speed = (currentGame == G_DESCENT_1 ? F1_0 * 50 : activeBMTable->robots[objp->id].max_speed[Difficulty_level]);
