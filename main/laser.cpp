...
{	
	switch( activeBMTable->weapons[obj->id].render_type )	
	{
	case WEAPON_RENDER_LASER:
		Int3();	// Not supported anymore!
					//Laser_draw_one(obj-Objects.data(), activeBMTable->weapons[obj->id].bitmap );
		break;
	case WEAPON_RENDER_BLOB:
		draw_object_blob(obj, activeBMTable->weapons[obj->id].bitmap  );
		break;
...
int create_weapon_object(int weapon_type,int segnum,vms_vector *position)
{
	...
	switch( activeBMTable->weapons[weapon_type].render_type )	
	{
		case WEAPON_RENDER_BLOB:
			...
			laser_radius = activeBMTable->weapons[weapon_type].blob_size;
			break;
		case WEAPON_RENDER_POLYMODEL:
	...
	if (activeBMTable->weapons[weapon_type].render_type == WEAPON_RENDER_POLYMODEL) 
	{
		obj->rtype.pobj_info.model_num = activeBMTable->weapons[obj->id].model_num;
		obj->size = fixdiv(activeBMTable->models[obj->rtype.pobj_info.model_num].rad,activeBMTable->weapons[obj->id].po_len_to_width_ratio);
	}

	obj->mtype.phys_info.mass = activeBMTable->weapons[weapon_type].mass;
	obj->mtype.phys_info.drag = activeBMTable->weapons[weapon_type].drag;
...
void create_omega_blobs(int firing_segnum, vms_vector *firing_pos, vms_vector *goal_pos, object *parent_objp)
{
	...
	for (i=0; i<num_omega_blobs; i++)
	{
		...
		if (segnum != -1) 
		{
			...
			objp->size = activeBMTable->weapons[objp->id].blob_size;

			objp->shields = fixmul(OMEGA_DAMAGE_SCALE*FrameTime, activeBMTable->weapons[objp->id].strength[Difficulty_level]);
	
			objp->ctype.laser_info.parent_type			= parent_objp->type;
			objp->ctype.laser_info.parent_signature	= parent_objp->signature;
			objp->ctype.laser_info.parent_num			= parent_objp-Objects.data();
	...
	{
		vm_vec_scale(&Objects[last_created_objnum].mtype.phys_info.velocity, activeBMTable->weapons[OMEGA_ID].speed[Difficulty_level]/4);
...
void do_omega_stuff(object *parent_objp, vms_vector *firing_pos, object *weapon_objp)
{
	...
	if (pnum == Player_num) 
	{
		...
		{
			obj_delete(weapon_objp-Objects.data());
			return;
		}
	...
	if ( parent_objp == Viewer )
		digi_play_sample( activeBMTable->weapons[weapon_objp->id].flash_sound, F1_0 );
	else
		digi_link_sound_to_pos( activeBMTable->weapons[weapon_objp->id].flash_sound, weapon_objp->segnum, 0, &weapon_objp->pos, 0, F1_0 );
	...
	if (lock_objnum == -1) 
	{
		...
		fq.p1						= &goal_pos;
		fq.rad					= 0;
		fq.thisobjnum			= parent_objp-Objects.data();
...
int Laser_create_new( vms_vector * direction, vms_vector * position, int segnum, int parent, int weapon_type, int make_sound )
{
	...
	if (activeBMTable->weapons[weapon_type].render_type == WEAPON_RENDER_POLYMODEL)
		laser_length = activeBMTable->models[obj->rtype.pobj_info.model_num].rad * 2;

	if (weapon_type == FLARE_ID)
		obj->mtype.phys_info.flags |= PF_STICK;		//this obj sticks to walls
	
	obj->shields = activeBMTable->weapons[obj->id].strength[Difficulty_level];
	
	// Fill in laser-specific data

	obj->lifeleft							= activeBMTable->weapons[obj->id].lifetime;
	...
	if ((obj->render_type == RT_POLYOBJ) || (activeBMTable->weapons[obj->id].homing_flag))
		vm_vector_2_matrix( &obj->orient,direction, &Objects[parent].orient.uvec ,NULL);
	...
	volume = F1_0;
	if (activeBMTable->weapons[obj->id].flash_sound > -1 )
	{
		if (make_sound)	
		{
			if ( parent == (Viewer-Objects.data()) )	
			{
				if (weapon_type == VULCAN_ID)	// Make your own vulcan gun  1/2 as loud.
					volume = F1_0 / 2;
				digi_play_sample( activeBMTable->weapons[obj->id].flash_sound, volume );
			} 
			else
			{
				digi_link_sound_to_pos( activeBMTable->weapons[obj->id].flash_sound, obj->segnum, 0, &obj->pos, 0, volume );
			}
		}
	}
	...
	if ((Objects[parent].type == OBJ_PLAYER) && (activeBMTable->weapons[weapon_type].render_type != WEAPON_RENDER_NONE) && (weapon_type != FLARE_ID)) 
	{
		vms_vector	end_pos;
		int			end_segnum;
		...
		if (end_segnum != obj->segnum) 
		{
			...
			{
				obj->pos = end_pos;
				obj_relink(obj-Objects.data(), end_segnum);
			} else
	...
	weapon_speed = activeBMTable->weapons[obj->id].speed[Difficulty_level];
	if (activeBMTable->weapons[obj->id].speedvar != 128) 
	{
		fix	randval;

		//	Get a scale factor between speedvar% and 1.0.
		randval = F1_0 - ((P_Rand() * activeBMTable->weapons[obj->id].speedvar) >> 6);
	...
	if (activeBMTable->weapons[obj->id].thrust != 0)
		weapon_speed /= 2;

	vm_vec_copy_scale( &obj->mtype.phys_info.velocity, direction, weapon_speed + parent_speed );

	//	Set thrust 
	if (activeBMTable->weapons[weapon_type].thrust != 0) 
	{
		obj->mtype.phys_info.thrust = obj->mtype.phys_info.velocity;
		vm_vec_scale(&obj->mtype.phys_info.thrust, fixdiv(activeBMTable->weapons[obj->id].thrust, weapon_speed+parent_speed));
	}

	if ((obj->type == OBJ_WEAPON) && (obj->id == FLARE_ID))
		obj->lifeleft += (P_Rand()-16384) << 2;		//	add in -2..2 seconds

	//	mprintf( 0, "Weapon speed = %.1f (%.1f)\n", f2fl(activeBMTable->weapons[obj->id].speed[Difficulty_level] + parent_speed), f2fl(parent_speed) );
...
int Laser_create_new_easy( vms_vector * direction, vms_vector * position, int parent, int weapon_type, int make_sound )
{
	...
	fq.p1						= position;
	fq.rad					= 0;
	fq.thisobjnum			= pobjp-Objects.data();
	fq.ignore_obj_list	= NULL;
...
int object_to_object_visibility(object *obj1, object *obj2, int trans_type)
{
	...
	fq.p1						= &obj2->pos;
	fq.rad					= 0x10;
	fq.thisobjnum			= obj1-Objects.data();
	fq.ignore_obj_list	= NULL;
	fq.flags					= trans_type;
...
int find_homing_object(vms_vector *curpos, object *tracker)
{
	...
	Assert((activeBMTable->weapons[tracker->id].homing_flag) || (tracker->id == OMEGA_ID));
	...
	else 
	{
		...
		if (tracker->ctype.laser_info.parent_num != Players[Player_num].objnum) 
		{
			if (!(Players[Player_num].flags & PLAYER_FLAGS_CLOAKED))
				best_objnum = ConsoleObject - Objects.data();
		} else 
		{
			int	window_num = -1;
			...
			for (i=Window_rendered_data[window_num].num_objects-1; i>=0; i--) 
			{
				...
				if (curobjp->type == OBJ_ROBOT) 
				{
					...
					//	Your missiles don't track your escort.
					if (activeBMTable->robots[curobjp->id].companion)
						if (tracker->ctype.laser_info.parent_type == OBJ_PLAYER)
							continue;
				}
				...
				if (dist < max_trackable_dist)
				{
					...
					// mprintf((0, "find_homing_object:  [%3i] %7.3f, min = %7.3f\n", curobjp-Objects.data(), f2fl(dot), f2fl(MIN_TRACKABLE_DOT)));
...
int track_track_goal(int track_goal, object *tracker, fix *dot)
{
	...
	if (object_is_trackable(track_goal, tracker, dot) && ((((tracker-Objects.data()) ^ FrameCount) % 8) != 0)) 
	{
		//mprintf((0, "ttg: QO"));
		return track_goal;
	}
	else if ((((tracker-Objects.data()) ^ FrameCount) % 4) == 0) 
	{
		int	rval = -2;
...
void Laser_player_fire_spread_delay(size_t objnum, int laser_type, int gun_num, fix spreadr, fix spreadu, fix delay_time, int make_sound, int harmless)
{
	...
	if (delay_time)
		vm_vec_scale_add2(&LaserPos, &obj->orient.fvec, -fixmul(delay_time, activeBMTable->weapons[laser_type].speed[Difficulty_level]));
...
//			return;		
	...
	size_t newObjnum = Laser_create_new( &LaserDir, &LaserPos, LaserSeg, objnum, laser_type, make_sound );
	obj = &Objects[objnum];
	...
	if (laser_type==GUIDEDMISS_ID && Multi_is_guided) 
	{
		...
		Guided_missile[obj->id]=&Objects[newObjnum];
	}
	...
	if (laser_type == CONCUSSION_ID ||
		...
		if (Missile_viewer == NULL && obj->id==Player_num)
			Missile_viewer = &Objects[newObjnum];

	//	If this weapon is supposed to be silent, set that bit!
	if (!make_sound)
		Objects[newObjnum].flags |= OF_SILENT;

	//	If this weapon is supposed to be silent, set that bit!
	if (harmless)
		Objects[newObjnum].flags |= OF_HARMLESS;
	...
	if ((obj == ConsoleObject) && ((Objects[newObjnum].id != PROXIMITY_ID) && (Objects[newObjnum].id != SUPERPROX_ID)))
		Player_fired_laser_this_frame = objnum;

	if (activeBMTable->weapons[laser_type].homing_flag) 
	{
		if (obj == ConsoleObject)
		{
			Objects[newObjnum].ctype.laser_info.track_goal = find_homing_object(&LaserPos, &Objects[newObjnum]);
			#ifdef NETWORK
			Network_laser_track = Objects[newObjnum].ctype.laser_info.track_goal;
			#endif
		}
		#ifdef NETWORK
		else // Some other player shot the homing thing
		{
			...
			Objects[newObjnum].ctype.laser_info.track_goal = Network_laser_track;
		}
		#endif
//		mprintf((0, "Selecting object #%i in find_homing_object_complete\n", Network_laser_track));
	}
...
{
	Laser_player_fire_spread_delay(objnum, laser_type, gun_num, spreadr, spreadu, 0, make_sound, harmless);
}


//	-----------------------------------------------------------------------------------------------------------
void Laser_player_fire(size_t objnum, int laser_type, int gun_num, int make_sound, int harmless)
{
	Laser_player_fire_spread(objnum, laser_type, gun_num, 0, 0, make_sound, harmless);
}
...
void Flare_create(object *obj)
{
	...
	energy_usage = activeBMTable->weapons[FLARE_ID].energy_usage;
...
void Laser_do_weapon_sequence(object *obj)
{
	...
	if (	!((FrameCount ^ obj->signature) & 3) &&
			(obj->id != FLARE_ID) &&
			(activeBMTable->weapons[obj->id].speed[Difficulty_level] > 0) &&
			(vm_vec_mag_quick(&obj->mtype.phys_info.velocity) < F2_0)) {
		obj_delete(obj-Objects.data());
		return;
	}
	...
	{		
		...
		vm_vec_scale(&obj->mtype.phys_info.velocity, activeBMTable->weapons[obj->id].speed[Difficulty_level]);
	}
	...
	if (obj->id == HELIX_ID) 
	{
		...
		speed = vm_vec_mag_quick(&obj->mtype.phys_info.velocity);
 		...
 		age = activeBMTable->weapons[obj->id].lifetime - obj->lifeleft;
	...
	if (activeBMTable->weapons[obj->id].homing_flag && !(obj->id==GUIDEDMISS_ID && obj==Guided_missile[Objects[obj->ctype.laser_info.parent_num].id] && obj->signature==Guided_missile[Objects[obj->ctype.laser_info.parent_num].id]->signature)) 
	{
		...
		if (obj->ctype.laser_info.creation_time + HOMING_MISSILE_STRAIGHT_TIME < GameTime)
		{
			...
			if (track_goal != -1) 
			{
				...
				max_speed = activeBMTable->weapons[obj->id].speed[Difficulty_level];
...
//mprintf((0, " dot=%5.2f ", f2fl(dot)));
				...
				if (activeBMTable->weapons[obj->id].render_type != WEAPON_RENDER_POLYMODEL)
					vm_vec_add2(&temp_vec, &vector_to_object);
...
//mprintf((0, "\n"));
	...
	if (CurrentLogicVersion >= LogicVer::FULL_1_0)
	{
		...
		if (weapon_speed > activeBMTable->weapons[obj->id].speed[Difficulty_level])
		{
			//	Only slow down if not allowed to move.  Makes sense, huh?  Allows proxbombs to get moved by physics force. --MK, 2/13/96
			if (activeBMTable->weapons[obj->id].speed[Difficulty_level]) 
			{
				fix	scale_factor;

				scale_factor = fixdiv(activeBMTable->weapons[obj->id].speed[Difficulty_level], weapon_speed);
				vm_vec_scale(&obj->mtype.phys_info.velocity, scale_factor);
			}
		}
	}
	else if (activeBMTable->weapons[obj->id].thrust)
	{
		weapon_speed = vm_vec_mag_quick(&obj->mtype.phys_info.velocity);
		if (weapon_speed > activeBMTable->weapons[obj->id].speed[Difficulty_level]) {
			fix	scale_factor;
...
int do_laser_firing_player(void)
{
	...
	energy_used = activeBMTable->weapons[weapon_index].energy_usage;
	...
	ammo_used = activeBMTable->weapons[weapon_index].ammo_usage;
...
//mprintf(0, ".");
			...
			plp->energy -= (energy_used * rval) / activeBMTable->weapons[weapon_index].fire_count;
			if (plp->energy < 0)
				plp->energy = 0;
...
int create_lightning_blobs(vms_vector direction, vms_vector start_pos, int start_segnum, int parent)
{
	...
	for (i = 0; i < num_blobs; i++) {
		...
		digi_play_sample(activeBMTable->weapons[obj->id].flash_sound, F1_0);
...
int do_laser_firing(int objnum, int weapon_num, int level, int flags, int nfires)
{
	...
	switch (weapon_num) 
	{
		case LASER_INDEX: 
		{
			...
			Laser_player_fire(objnum, weapon_num, 0, 1, 0);
			Laser_player_fire(objnum, weapon_num, 1, 0, 0);

			if (flags & LASER_QUAD) 
			{
				//	hideous system to make quad laser 1.5x powerful as normal laser, make every other quad laser bolt harmless
				Laser_player_fire(objnum, weapon_num, 2, 0, 0);
				Laser_player_fire(objnum, weapon_num, 3, 0, 0);
			}
			break;
		}
		case VULCAN_INDEX: 
		{
			...
			//if (P_Rand() > 24576)
			//	make_sound = 1;
			Laser_player_fire_spread(objnum, VULCAN_ID, 6, P_Rand()/8 - 32767/16, P_Rand()/8 - 32767/16, make_sound, 0);
			if (nfires > 1) 
			{
				Laser_player_fire_spread(objnum, VULCAN_ID, 6, P_Rand()/8 - 32767/16, P_Rand()/8 - 32767/16, 0, 0);
				if (nfires > 2)
				{
					Laser_player_fire_spread(objnum, VULCAN_ID, 6, P_Rand()/8 - 32767/16, P_Rand()/8 - 32767/16, 0, 0);
				}
		...
		case SPREADFIRE_INDEX:
			...
			{
				Laser_player_fire_spread(objnum, SPREADFIRE_ID, 6, F1_0/16, 0, 0, 0);
				Laser_player_fire_spread(objnum, SPREADFIRE_ID, 6, -F1_0/16, 0, 0, 0);
				Laser_player_fire_spread(objnum, SPREADFIRE_ID, 6, 0, 0, 1, 0);
			} 
			else 
			{
				Laser_player_fire_spread(objnum, SPREADFIRE_ID, 6, 0, F1_0/16, 0, 0);
				Laser_player_fire_spread(objnum, SPREADFIRE_ID, 6, 0, -F1_0/16, 0, 0);
				Laser_player_fire_spread(objnum, SPREADFIRE_ID, 6, 0, 0, 1, 0);
			}
			break;
		...
		case PLASMA_INDEX:
			Laser_player_fire(objnum, PLASMA_ID, 0, 1, 0);
			Laser_player_fire(objnum, PLASMA_ID, 1, 0, 0);
			if (nfires > 1) 
			{
				Laser_player_fire_spread_delay(objnum, PLASMA_ID, 0, 0, 0, FrameTime/2, 1, 0);
				Laser_player_fire_spread_delay(objnum, PLASMA_ID, 1, 0, 0, FrameTime/2, 0, 0);
			}
			break;
...
//			mprintf((0, "Fusion multiplier %f.\n", f2fl(Fusion_charge)));
			...
			Laser_player_fire(objnum, FUSION_ID, 0, 1, 0);
			Laser_player_fire(objnum, FUSION_ID, 1, 1, 0);
		...
		case SUPER_LASER_INDEX: 
		{
			...
			Laser_player_fire(objnum, super_level, 0, 1, 0);
			Laser_player_fire(objnum, super_level, 1, 0, 0);

			if (flags & LASER_QUAD)
			{
				//	hideous system to make quad laser 1.5x powerful as normal laser, make every other quad laser bolt harmless
				Laser_player_fire(objnum, super_level, 2, 0, 0);
				Laser_player_fire(objnum, super_level, 3, 0, 0);
			}
			break;
		}
		case GAUSS_INDEX: 
		{
			...
			Laser_player_fire_spread(objnum, GAUSS_ID, 6, (P_Rand()/8 - 32767/16)/5, (P_Rand()/8 - 32767/16)/5, make_sound, 0);
			if (nfires > 1)
			{
				Laser_player_fire_spread(objnum, GAUSS_ID, 6, (P_Rand()/8 - 32767/16)/5, (P_Rand()/8 - 32767/16)/5, 0, 0);
				if (nfires > 2) 
				{
					Laser_player_fire_spread(objnum, GAUSS_ID, 6, (P_Rand()/8 - 32767/16)/5, (P_Rand()/8 - 32767/16)/5, 0, 0);
				}
		...
		case HELIX_INDEX: 
		{
			...
			Laser_player_fire_spread(objnum, HELIX_ID, 6,  0,  0, 1, 0);
			Laser_player_fire_spread(objnum, HELIX_ID, 6,  spreadr,  spreadu, 0, 0);
			Laser_player_fire_spread(objnum, HELIX_ID, 6, -spreadr, -spreadu, 0, 0);
			Laser_player_fire_spread(objnum, HELIX_ID, 6,  spreadr*2,  spreadu*2, 0, 0);
			Laser_player_fire_spread(objnum, HELIX_ID, 6, -spreadr*2, -spreadu*2, 0, 0);
			break;
		}

		case PHOENIX_INDEX:
			Laser_player_fire(objnum, PHOENIX_ID, 0, 1, 0);
			Laser_player_fire(objnum, PHOENIX_ID, 1, 0, 0);
			if (nfires > 1)
			{
				Laser_player_fire_spread_delay(objnum, PHOENIX_ID, 0, 0, 0, FrameTime/2, 1, 0);
				Laser_player_fire_spread_delay(objnum, PHOENIX_ID, 1, 0, 0, FrameTime/2, 0, 0);
			}
			break;
...
int create_homing_missile(object *objp, int goal_obj, int objtype, int make_sound)
{
	...
	objnum = Laser_create_new(&vector_to_goal, &objp->pos, objp->segnum, objp-Objects.data(), objtype, make_sound);
	if (objnum == -1)
		return -1;
...
// -- DEBUG --
	...
	{
		Assert(activeBMTable->weapons[objp->id].children != -1);
	}
// -- DEBUG --

	if (((objp->type == OBJ_WEAPON) 
		&& ((CurrentLogicVersion >= LogicVer::FULL_1_0 && activeBMTable->weapons[objp->id].children != -1) 
		...
		|| (objp->type == OBJ_ROBOT)) 
	{
		...
		if (currentGame == G_DESCENT_1 && parent_type == OBJ_ROBOT) {
			...
			if (objp->type == OBJ_WEAPON)
			{
				blob_id = activeBMTable->weapons[objp->id].children;
...
void do_missile_firing(int do_autoselect)
{
	...
	if (Guided_missile[Player_num] && Guided_missile[Player_num]->signature==Guided_missile_sig[Player_num]) 
	{
		...
		Next_missile_fire_time = GameTime + activeBMTable->weapons[Secondary_weapon_to_weapon_info[Secondary_weapon]].fire_wait;
		return;
	}
	...
	if (!Player_is_dead && (Players[Player_num].secondary_ammo[Secondary_weapon] > 0))	
	{
		...
		Laser_player_fire( ConsoleObject - Objects.data(), weapon_id, weapon_gun, 1, 0);
