...
std::vector<ai_local> Ai_local_info(MAX_OBJECTS);
...
int ai_behavior_to_mode(int behavior)
{
	switch (behavior) {
		...
		default:	//disimbaguate identically-valued behaviors
...
{
	Boss_hit_time = (currentGame == G_DESCENT_2 ? -F1_0*10 : 0);
}
...
void init_ai_object(int objnum, int behavior, int hide_segment)
{
    ...
    // [DW] SNIPE = FOLLOW_PATH, so no need for game check
	if ((behavior == AIB_SNIPE) || (behavior == AIB_STATION) || (behavior == AIB_RUN_FROM) || (behavior == AIB_FOLLOW) || (currentGame == G_DESCENT_1 && behavior == AIB_HIDE)) {
...
void init_ai_objects(void)
{
	...
	Boss_dying = 0;
	Boss_been_hit = 0;
	Gate_interval = (currentGame == G_DESCENT_1
        ? F1_0*5 - Difficulty_level * F1_0 / 2
        : F1_0*4 - Difficulty_level * i2f(2) / 3
    );
...
int create_gated_robot( int segnum, int object_id, vms_vector* pos)
{
	...
	robot_info	*robptr = &activeBMTable->robots[object_id];
	int		i, count=0;
	fix		objsize = activeBMTable->models[robptr->model_num].rad;
	...
	if (count > 2*Difficulty_level + 3 * (currentGame == G_DESCENT_2 ? 2 : 1)) {
...
void ai_turn_towards_vector(vms_vector goal_vector, object *objp, fix rate)
{
	...
	if (Seismic_tremor_magnitude) {
		...
		scale = fixdiv(2*Seismic_tremor_magnitude, activeBMTable->robots[objp->id].mass);
...
int do_silly_animation(object *objp)
{
	...
	robot_type = objp->id;
	num_guns = activeBMTable->robots[robot_type].n_guns;
	...
	for (gun_num=0; gun_num <= num_guns; gun_num++) {
		...
		for (joint=0; joint<num_joint_positions; joint++) {
			...
			if (jointnum >= activeBMTable->models[objp->rtype.pobj_info.model_num].n_models) {
...
void ai_frame_animation(object *objp)
{
	...
	num_joints = activeBMTable->models[objp->rtype.pobj_info.model_num].n_models;
...
void ai_move_relative_to_player(object *objp, ai_local *ailp, fix dist_to_player, vms_vector vec_to_player, fix circle_distance, int evade_only, int player_visibility)
{
	...
	if (objp->ctype.ai_info.danger_laser_num > 0 && objp->ctype.ai_info.danger_laser_num < Objects.size()) {
		...
		if ((dobjp->type == OBJ_WEAPON) && (dobjp->signature == objp->ctype.ai_info.danger_laser_signature)) {
			...
			field_of_view = activeBMTable->robots[objp->id].field_of_view[Difficulty_level];
			...
			if ((dot > field_of_view) || (robptr->companion)) {
				...
				if ((laser_robot_dot > F1_0*7/8) && (dist_to_laser < F1_0*80)) {
					...
					ai_evaded = 1;
					evade_speed = activeBMTable->robots[objp->id].evade_speed[Difficulty_level];
	...
	if (robptr->attack_type == 1) {
		...
		fix distanceThreshold = (currentGame == G_DESCENT_2 
			? ((3+objval)*circle_distance/2)
			: circle_distance * 2
		);
		...
		else if (dist_to_player < distanceThreshold && (ailp->next_fire > -F1_0 || currentGame == G_DESCENT_1)) {
			move_around_player(objp, vec_to_player, -1);
		} else {
			if (currentGame == G_DESCENT_2 && ((-ailp->next_fire > F1_0 + (objval << 12)) && player_visibility)) {
...
{
	if ((currentGame == G_DESCENT_2 && Dist_to_last_fired_upon_player_pos < FIRE_AT_NEARBY_PLAYER_THRESHOLD ) || (player_visibility >= 1)) {
	...
	if (!*flag) {
		if (Players[Player_num].flags & PLAYER_FLAGS_CLOAKED) {
			...
			*player_visibility = player_is_visible_from_object(objp, pos, robptr->field_of_view[Difficulty_level], *vec_to_player);
			...
			if ((ailp->next_misc_sound_time < GameTime) && ((ailp->next_fire < F1_0) || (currentGame == G_DESCENT_2 && ailp->next_fire2 < F1_0)) && (dist < F1_0*20)) {
				// mprintf((0, "ANGRY! "));
			...
			*player_visibility = player_is_visible_from_object(objp, pos, robptr->field_of_view[Difficulty_level], *vec_to_player);
			...
			if ((currentGame == G_DESCENT_2 || !Player_exploded) && (ailp->previous_visibility != *player_visibility) && (*player_visibility == 2)) {
		...
		*flag = 1;
		...
		if (currentGame == G_DESCENT_2 && ailp->player_awareness_type >= PA_NEARBY_ROBOT_FIRED)
...
int ai_door_is_openable(object *objp, segment *segp, int sidenum)
{
	...
	if (currentGame == G_DESCENT_2 && ((objp == NULL) || (activeBMTable->robots[objp->id].companion == 1))) {
		int	ailp_mode;
		...
		if ((ailp_mode != AIM_GOTO_PLAYER) && (wallp->controlling_trigger != -1)) {
			...
			if (clip_num == -1)
				return 1;
			else if (activeBMTable->wclips[clip_num].flags & WCF_HIDDEN) {
		...
		if (wallp->type == WALL_DOOR)  {
			if (wallp->type == WALL_BLASTABLE)
				...
				else if ((ailp_mode != AIM_GOTO_PLAYER) && (activeBMTable->wclips[clip_num].flags & WCF_HIDDEN)) {
					if (wallp->state == WALL_DOOR_CLOSED)
						return 0;
					else
						return 1;
	...
	} else if ((objp->id == ROBOT_BRAIN) || (objp->ctype.ai_info.behavior == AIB_RUN_FROM) || (currentGame == G_DESCENT_2 && objp->ctype.ai_info.behavior == AIB_SNIPE)) {
		if (wall_num != -1)
			if ((wallp->type == WALL_DOOR) && (wallp->keys == KEY_NONE) && !(wallp->flags & WALL_DOOR_LOCKED))
				return 1;
			else if (currentGame == G_DESCENT_2 && wallp->keys != KEY_NONE) {	//	Allow bots to open doors to which player has keys.
...
int openable_doors_in_segment(int segnum)
{
	...
	for (i=0; i<MAX_SIDES_PER_SEGMENT; i++) {
		if (Segments[segnum].sides[i].wall_num != -1) {
			...
			if ((Walls[wall_num].type == WALL_DOOR) && (Walls[wall_num].keys == KEY_NONE) && (Walls[wall_num].state == WALL_DOOR_CLOSED) && (currentGame == G_DESCENT_1 || !(Walls[wall_num].flags & WALL_DOOR_LOCKED) && !(activeBMTable->wclips[Walls[wall_num].clip_num].flags & WCF_HIDDEN)))
				return i;
		}
	}
...
void ai_do_actual_firing_stuff(object *obj, ai_static *aip, ai_local *ailp, robot_info *robptr, vms_vector vec_to_player, fix dist_to_player, vms_vector gun_point_in, int player_visibility, int object_animates, int gun_num)
{
	fix	dot;
	size_t objnum = obj - Objects.data();

	vms_vector* gun_point = &gun_point_in; //Gun point gets recalculated frequently, but still want to use object reallocation safe parameters
	...
	if ((player_visibility == 2) || (currentGame == G_DESCENT_2 && Dist_to_last_fired_upon_player_pos < FIRE_AT_NEARBY_PLAYER_THRESHOLD )) {
		...
		if (!object_animates || ready_to_fire(robptr, ailp)) {
			...
			if ((dot >= 7*F1_0/8) || (currentGame == G_DESCENT_2 && (dot > F1_0/4) &&  robptr->boss_flag)) {

				if (gun_num < activeBMTable->robots[obj->id].n_guns) {
					if (robptr->attack_type == 1) {
						...
						if ((gun_point->x == 0) && (gun_point->y == 0) && (gun_point->z == 0)) {
							...
							if (currentGame == G_DESCENT_2) {
								...
								ai_fire_laser_at_player(obj, *gun_point, 0, vmd_zero_vector);
								obj = &Objects[objnum];
							}
				...
				if (aip->CURRENT_GUN >= activeBMTable->robots[obj->id].n_guns)
					if ((activeBMTable->robots[obj->id].n_guns == 1) || (activeBMTable->robots[obj->id].weapon_type2 == -1))
						aip->CURRENT_GUN = 0;
					else
						aip->CURRENT_GUN = 1;
			}
		}
	} else if (!robptr->attack_type && (activeBMTable->weapons[activeBMTable->robots[obj->id].weapon_type].homing_flag == 1) || (currentGame == G_DESCENT_2 && ((activeBMTable->robots[obj->id].weapon_type2 != -1) && (activeBMTable->weapons[activeBMTable->robots[obj->id].weapon_type2].homing_flag == 1)))) {
...
void pae_aux(int segnum, int type, int level)
{
	...
	int type_sub = (type == 4 ? 1 : 0);

	// Process children.
	if (level <= (currentGame == G_DESCENT_1 ? 4 : 3))
		for (j = 0; j < MAX_SIDES_PER_SEGMENT; j++)
			if (IS_CHILD(Segments[segnum].children[j]))
					pae_aux(Segments[segnum].children[j], type - type_sub, level + 1);
}
...
void set_player_awareness_all(void)
{
	...
	for (i = 0; i <= Highest_object_index; i++)
		if (Objects[i].control_type == CT_AI && Objects[i].segnum >= 0) {
			...
			if (currentGame == G_DESCENT_2 && New_awareness[Objects[i].segnum] > Ai_local_info[i].player_awareness_type)
...
extern void set_next_fire_time_d1(object *objp, ai_local *ailp, robot_info *robptr, int gun_num);
extern void set_next_fire_time_d2(object *objp, ai_local *ailp, robot_info *robptr, int gun_num);

void set_next_fire_time(object *objp, ai_local *ailp, robot_info *robptr, int gun_num) {
	if (currentGame == G_DESCENT_1)
		set_next_fire_time_d1(objp, ailp, robptr, gun_num);
	else if (currentGame == G_DESCENT_2) 
		set_next_fire_time_d2(objp, ailp, robptr, gun_num);
	else
		Int3();
}

extern void ai_fire_laser_at_player_d1(object *obj, vms_vector fire_point, int gun_num, vms_vector believed_player_pos);
extern void ai_fire_laser_at_player_d2(object *obj, vms_vector fire_point, int gun_num, vms_vector believed_player_pos);

void ai_fire_laser_at_player(object *obj, vms_vector fire_point, int gun_num, vms_vector believed_player_pos) {
	if (currentGame == G_DESCENT_1)
		ai_fire_laser_at_player_d1(obj, fire_point, gun_num, believed_player_pos);
	else if (currentGame == G_DESCENT_2)
		ai_fire_laser_at_player_d2(obj, fire_point, gun_num, believed_player_pos);
	else
		Int3();
}

extern void do_ai_robot_hit_d1(object *objp, int type);
extern void do_ai_robot_hit_d2(object *objp, int type);

void do_ai_robot_hit(object *objp, int type) {
	if (currentGame == G_DESCENT_1)
		do_ai_robot_hit_d1(objp, type);
	else if (currentGame == G_DESCENT_2)
		do_ai_robot_hit_d2(objp, type);
	else
		Int3();
}
...
extern void do_boss_stuff_d1(object *objp);
extern void do_boss_stuff_d2(object *objp, int player_visibility);

void do_boss_stuff(object *objp, int player_visibility) {
	if (currentGame == G_DESCENT_1)
		do_boss_stuff_d1(objp);
	else if (currentGame == G_DESCENT_2)
		do_boss_stuff_d2(objp, player_visibility);
	else
		Int3();
}
