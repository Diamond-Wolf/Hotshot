...
int save_mine_data_compiled(FILE * SaveFile)
{
	...
	{
		char	message[128];
		snprintf(message, 128, "Error: Too many segments (%i > %i) for game (not editor)", Highest_segment_index+1, MAX_GAME_SEGMENTS);
	...
	{
		char	message[128];
		snprintf(message, 128, "Error: Too many vertices (%i > %i) for game (not editor)", Highest_vertex_index+1, MAX_GAME_VERTICES);
		MessageBox( -2, -2, 1, message, "Ok" );
	}
